...
**Status:** Hardening-stage governed blockchain kernel - v0.3.0. Protocol spec frozen ([PROTOCOL.md](PROTOCOL.md)). Single-node reference runtime with optional p2p alpha, persistent block log, encrypted validator keystore, genesis-embedded consensus params, periodic snapshots, and 621 tests in the current workspace listing. New chains default to block version 2, where validator liquidity lives under the canonical agent account; block version 1 replay remains supported for legacy compatibility. CI is green on Ubuntu, Windows, and the security audit job. Canonical status note: [docs/STATUS.md](docs/STATUS.md).
...
- Hardening posture: 617 tests, CI green on Ubuntu + Windows + security audit.
...
- **Replay-authoritative state:** Blocks, metadata, encrypted validator keys, and periodic snapshots persist across restarts, but there is no fully durable state database.
...
- **ContractInvoke namespace is loose:** Maps to both `contract/` and `data/`; should tighten to per-contract namespace.
